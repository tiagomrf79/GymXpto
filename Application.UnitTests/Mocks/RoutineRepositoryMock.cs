...
{
    public static Mock<IAsyncRepository<Routine>> GetRoutineRepository()
    {
        ...
        var mockRoutineRepository = new Mock<IAsyncRepository<Routine>>();

        mockRoutineRepository.Setup(repo => repo.GetByIdAsync(It.IsAny<Guid>())).ReturnsAsync(
            (Guid id) =>
                routines.FirstOrDefault(r => r.Id == id)
            );
        ...
        mockRoutineRepository.Setup(repo => repo.UpdateAsync(It.IsAny<Routine>())).Returns(
            (Routine routine) =>
            {
                var routineToUpdate = routines.FirstOrDefault(r => r.Id == routine.Id);
